...
**Custom domain: PREPARED, NOT ACTIVE (15 Sep 2026).** `book.saltycowboybali.com` was attached in
4d39fe2 and **reverted the same day**, because the Wix DNS record it needs was not in place and
attaching a domain makes Pages redirect the github.io address to it, taking the site down in the gap.
There is deliberately **no `CNAME` file in this repo right now.** The live address is the github.io one
above.

To finish it later: add the DNS record FIRST, then recreate `CNAME` containing one line,
`book.saltycowboybali.com`, and push. Order matters; reversing it causes an outage.

The main site `saltycowboybali.com` is **Wix**, and Wix also runs the DNS (`ns0`/`ns1.wixdns.net`), so
the record lives in the Wix dashboard, not at a registrar. The apex and `www` are Wix's own and must not
be touched. Only the `book` subdomain is ours:

```
Type: CNAME   Host: book   Value: threshold-00.github.io
```

Note this needs DNS-record access in the Wix domain settings, which is not the same thing as admin on
the site editor. Someone can be a full site admin and still not see DNS.
...
two addresses. This was verified at 4d39fe2 and is the reason the switch needs no code change.
