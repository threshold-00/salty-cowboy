...
- **"I'm booking for someone else" checkbox** (29 Aug 2026 BOOKING-FOR-OTHER): riding activities only (Beach & Rice Field Ride, Insta Ride, Dressage Masterclass, i.e. every activity with `riding: true`), rendered at the end of the "Who's coming?" section as a sibling right after the photographer add-on block (photoshoots and riding activities never overlap, so the two never render together). Unchecked by default; resets to unchecked both explicitly on every activity-change path and defensively whenever `isRiding` goes false. When checked, reveals a note asking the booker to check the weight of the person they're booking for, reiterating the 75kg guideline and no-refund policy, styled like the existing weight-warning blocks. When checked, adds one line to the WhatsApp payload after the "Group size" line: "Booking on behalf of another rider. 75kg weight guideline shown to the person booking." Unchecked, the payload is unaffected. Not shown for photoshoots or non-riding lessons.
