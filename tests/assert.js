...
has('photo_ricefield deliberately has no image field yet (no source photo provided for it), falls back to the plain placeholder', 'id: "photo_ricefield",\n    durations: ["1hr", "1.5hr", "2hr", "3hr"],\n    maxPeople: 5,\n    photoshoot: true\n  }');
has('photo_paddock carries a real image path', 'photoshoot: true,\n    image: "images/photo-paddock.avif"');
...
has('insta 1hr/1.5hr prices still match beach ride, 2hr repriced (PRICE-INSTARIDE commit, 2,400,000 -> 2,700,000, no longer the same linear per-half-hour rate as beach ride)', 'insta: [{ l: "1 hr", v: "IDR 1,600,000" }, { l: "1.5 hr", v: "IDR 2,000,000" }, { l: "2 hr", v: "IDR 2,700,000" }]');
has('insta 1hr/1.5hr prices still match beach ride, 2hr repriced (id)', 'insta: [{ l: "1 jam", v: "IDR 1,600,000" }, { l: "1,5 jam", v: "IDR 2,000,000" }, { l: "2 jam", v: "IDR 2,700,000" }]');
has('insta 1hr/1.5hr prices still match beach ride, 2hr repriced (ru)', 'insta: [{ l: "1 ч", v: "IDR 1,600,000" }, { l: "1,5 ч", v: "IDR 2,000,000" }, { l: "2 ч", v: "IDR 2,700,000" }]');
...
has('photo_paddock carries hideCardImage: true, the only activity that does (PADDOCK-IMG commit) - card falls back to the grey placeholder while the Step 2 header band, which reads actObj.image directly and never checks this flag, keeps showing the photo', 'imagePosition: "center 70%",\n    // PADDOCK-IMG commit');
...
has('gallery-row (4 placeholder thumbnails) renders for isPhotoshoot activities only, right where the image band used to sit; each thumb opens the lightbox at its own index on click', 'isPhotoshoot && /*#__PURE__*/React.createElement("div", {\n    className: "gallery-row"\n  }, [0, 1, 2, 3].map(i => /*#__PURE__*/React.createElement("div", {\n    key: i,\n    className: "gallery-thumb",\n    onClick: () => setLightboxIndex(i)\n  })))');
...
has('clicking the photo itself does not close the lightbox (stops propagation before it can reach the backdrop)', 'className: "gallery-lightbox-photo",\n    onClick: e => e.stopPropagation()');
...
has('handleSend / buildWhatsAppMessage payload fields: totalPriceStr deliberately added (Ro\'s explicit request to include the final price), the one sanctioned exception to "never alter the payload shape" - every other field is exactly as before', 'const message = buildWhatsAppMessage({\n      actObj,\n      durationLabel: duration,\n      formattedDates,\n      isCourse,\n      selectedTime,\n      numPeople,\n      riders,\n      showWeight,\n      notes,\n      photographerTier: isPhotoshoot ? photographerTier : null,\n      grooming: needsGrooming ? grooming : null,\n      totalPriceStr\n    });');
...
has('photo_paddock\'s imagePosition tuned to "center 70%" - a plain centered crop cut the rider and horse (both positioned low in the source photo) out of the desktop band entirely, showing only trees and a cottage roof', 'image: "images/photo-paddock.avif",\n    imagePosition: "center 70%"');
...
has('final price line reads T.en.totalCostLabel ("Total cost"), matching the site\'s own English label text rather than a new hardcoded string, sits right after Group size and before the blank line into *Riders:*, and is skipped entirely if totalPriceStr is falsy (e.g. duration/numPeople/selPrice not all chosen yet)', 'lines.push("Group size: " + numPeople);\n  if (totalPriceStr) lines.push(T.en.totalCostLabel + ": " + totalPriceStr);\n  lines.push("");\n  lines.push("*Riders:*");');
...
has('section 2 box closes right after addon-group, then the section-2 blocked-note, immediately before section 3\'s own top-level div begins (section 3 now always renders, batch 7: section 3 accordion)', '")")))))), blockedSection === 2 && /*#__PURE__*/React.createElement("div", {\n    className: "blocked-note"\n  }, /*#__PURE__*/React.createElement("span", null, t.section2Blocked), /*#__PURE__*/React.createElement("button", {\n    className: "blocked-note-close",\n    type: "button",\n    onClick: () => setBlockedSection(null)\n  }, "✕")), screen === "riders" && /*#__PURE__*/React.createElement("div", {\n    className: "step2-section-box fu step2-block"');
