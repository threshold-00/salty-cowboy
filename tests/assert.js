...
has('toggleSection2/toggleSection3 helpers introduced deliberately this time (batch 10: blocked-note), so the open-ahead-of-sequence check lives in one place instead of duplicated across the heading and hint paragraph', 'function toggleSection2() {\n    if (section2Collapsed && !section1Complete) {\n      setBlockedSection(2);\n    } else {\n      setSection2Override(!section2Collapsed);\n    }\n  }\n  function toggleSection3() {\n    if (section3Collapsed && !datesComplete) {\n      setBlockedSection(3);\n    } else {\n      setSection3Override(!section3Collapsed);\n    }\n  }');
...
has('section3Collapsed is collapsed whenever dates aren\'t complete yet (default), overridden by section3Override once the user has toggled it', 'const section3Collapsed = section3Override !== null ? section3Override : !datesComplete;');
has('section3Collapsed reset effect: when dates stop being complete, section3Override resets to null (falls back to the default-collapsed state)', 'useEffect(() => {\n    if (!datesComplete) setSection3Override(null);\n  }, [datesComplete]);');
...
has('en section2Blocked/section3Blocked keys are precise to each section\'s actual gate: section 2 needs dates AND time (section1Complete), section 3 only needs dates (datesComplete)', 'section2Blocked: "Please pick a date and time first",\n    section3Blocked: "Please pick a date first",');
has('id section2Blocked/section3Blocked keys', 'section2Blocked: "Silakan pilih tanggal dan waktu terlebih dahulu",\n    section3Blocked: "Silakan pilih tanggal terlebih dahulu",');
has('ru section2Blocked/section3Blocked keys', 'section2Blocked: "Пожалуйста, сначала выберите дату и время",\n    section3Blocked: "Пожалуйста, сначала выберите дату",');
