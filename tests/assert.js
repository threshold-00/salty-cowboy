...
has('section-3 confirmation-notice/blocked-note pair now runs straight into cost-funds-card, with no rider-info card between them (10 Sep 2026 CHECKBOXES-UNDER-WHOS-COMING moved it back into section 2)', 't.notice)), blockedSection === 3 && /*#__PURE__*/React.createElement("div", {\n    className: "blocked-note"\n  }, /*#__PURE__*/React.createElement("span", null, t.section3Blocked), /*#__PURE__*/React.createElement("button", {\n    className: "blocked-note-close",\n    type: "button",\n    onClick: () => setBlockedSection(null)\n  }, "✕")), screen === "riders" && /*#__PURE__*/React.createElement("div", {\n    className: "cost-funds-card fu step2-block"');
has('cost-funds-card sits directly after the section-3 blocked-note and before the confirm screen', '"✕")), screen === "riders" && /*#__PURE__*/React.createElement("div", {\n    className: "cost-funds-card fu step2-block"');
...
has('BOTH optional extras (booking-for-other and the photographer add-on) now sit inside section 2 and hide with section2Collapsed (10 Sep 2026 CHECKBOXES-UNDER-WHOS-COMING). They are mutually exclusive in practice, since isRiding and isPhotoshoot never overlap, so only one is ever on screen', 'className: "addon-group",\n    style: {\n      display: section2Collapsed ? "none" : undefined\n    }\n  }', 2);
has('photographer add-on addon-group followed by the addon-title div', 'className: "addon-group",\n    style: {\n      display: section2Collapsed ? "none" : undefined\n    }\n  }, /*#__PURE__*/React.createElement("div", {\n    className: "addon-title"\n  }, t.addonTitle)');
has('booking-for-other wrapper is addon-group inside section 2, not its own step2-section-box card, followed directly by its own perm-row with no addon-title', 'className: "addon-group",\n    style: {\n      display: section2Collapsed ? "none" : undefined\n    }\n  }, /*#__PURE__*/React.createElement("div", {\n    className: "perm-row addon-row",\n    onClick: () => {\n      const next = !bookingForOther;\n      setBookingForOther(next);\n      if (next) setShowRiderInfoModal(true);\n    }\n  }');
...
has('defensive reset when isRiding goes false, on top of the activity-change reset calls', 'useEffect(() => {\n    if (!canBookForOther) setBookingForOther(false);\n  }, [canBookForOther]);');
has('booking-for-other gated on isRiding && riders.length > 0, dropping the screen === "riders" half it needed as a standalone card, since section 2 is already inside a screen === "riders" block (10 Sep 2026 CHECKBOXES-UNDER-WHOS-COMING)', '}), canBookForOther && riders.length > 0 && /*#__PURE__*/React.createElement("div", {\n    className: "addon-group"');
has('booking-for-other sits ahead of the photographer add-on, both as the last children of section 2', 't.riderInfoReopenNote)))), isPhotoshoot && riders.length > 0 && /*#__PURE__*/React.createElement("div", {\n    className: "addon-group"');
...
has('bookingForOther gated on isRiding in handleSend, mirroring the photographerAddon/isPhotoshoot pattern', 'bookingForOther: canBookForOther ? bookingForOther : false,');

// ─── 11 Sep 2026 LESSONS-BOOK-FOR-OTHER + grooming_type rename ───────────────
has('canBookForOther opens booking-on-behalf to Rides AND Lessons, not Photoshoots (Ro, 11 Sep 2026). dressage already had it via riding: true; this adds joinup, whisper, masterclass and groupclinic', 'const canBookForOther = !isPhotoshoot;');
missing('nothing gates booking-for-other on isRiding any more', 'isRiding ? bookingForOther : false');
// canBookForOther and isPhotoshoot are exact complements, so the two addon-group
// checkboxes still never appear together; the count==2 assertion above still holds.
has('the copyable rider-info message drops the 75kg weight section where weight is never collected, so a groundwork Lesson does not carry a rider weight limit', 'if (showWeight) {\n    lines.push(t.riderInfoWeightTitle + ": " + t.riderInfoWeightBody);');
has('the rider-info MODAL gates the same weight section on showWeight, so the on-screen copy matches the copyable message', 't.riderInfoArrivalBody)), showWeight && /*#__PURE__*/React.createElement("div", {\n    className: "rider-info-section"');
has('the WhatsApp line to Simone says "another rider" with the weight guideline only where weight was collected, and "someone else" otherwise', ': "Booking on behalf of someone else. Arrival and cost info shown to the person booking.");');
has('the log column is grooming_type, naming the wet/dry STYLE rather than implying "did they groom"', 'grooming_type: needsGrooming ? grooming : null,');
has('the copyable rider-info message lists ALL course dates, not formattedDates[0]: opening this flow to Lessons put the 3-day whisper course through it', 'if (formattedDates.length > 1) {\n      lines.push(t.courseDaysTitle + ": " + formattedDates.join(" \u00b7 "));\n      if (selectedTime) lines.push(t.timeSlot + ": " + selectedTime);');
missing('no unguarded formattedDates[0] left in the rider-info message', 'lines.push(t.sDateTime + ": " + formattedDates[0] + (selectedTime ? " \u00b7 " + selectedTime : ""));\n  }');
// courseDaysTitle and timeSlot are the keys reused for the course branch; both
// must exist in all three languages or the message renders "undefined: ...".
has('courseDaysTitle exists in all three languages', 'courseDaysTitle:', 3);
has('timeSlot exists in all three languages', 'timeSlot:', 3);
missing('no bare grooming key is left in the log row', 'grooming: needsGrooming ? grooming : null,\n      total_price');
...
has('riderInfoMessage includes arrival, money-goes and (only where weight is collected) the weight section, reusing t.costFundsTitle/t.moneyGoesBody rather than duplicating the funds copy', 'lines.push(t.riderInfoArrivalTitle + ": " + t.riderInfoArrivalBody);\n  lines.push("");\n  // Weight guidance only where weight is actually collected. Most Lessons are\n  // groundwork, so a 75kg rider limit in the message would be nonsense there.\n  if (showWeight) {\n    lines.push(t.riderInfoWeightTitle + ": " + t.riderInfoWeightBody);\n    lines.push("");\n  }\n  lines.push(t.costFundsTitle + ": " + t.moneyGoesBody);');
has('riderInfoMessage computed once per render as a derived const, shared by the modal display and the copy handler', 'const riderInfoMessage = buildRiderInfoMessage({ t, actObj, formattedDates, selectedTime, showWeight });');
...
has('logEvent swallows everything else', '} catch (e) {\n    console.warn("[log] skipped:", e && e.message);\n  }');

// This is the ONLY defence for the PII whitelist. assert.js counts substrings
...
has('duration logs the raw state value so the column stays groupable in a pivot', 'duration: duration || "",');

has('ref is generated per booking from an alphabet with no I/O/0/1, since it gets read off a phone screen', 'const alphabet = "ABCDEFGHJKLMNPQRSTUVWXYZ23456789";');
has('the ref is appended to the outbound message so a sheet row can be matched back to Simone\'s WhatsApp thread', 'const ref = newRef();\n    const outbound = message + "\\n_Ref: " + ref + "_";');
has('logEvent fires after openWhatsApp (popup stays inside the user-gesture window) and before setScreen', 'openWhatsApp(outbound);\n    logEvent("booking", () => bookingLogRow(ref));\n    setScreen("confirm");');
has('the buildWhatsAppMessage call itself is untouched by the log: message is built first, then the ref is appended to a separate value', 'const outbound = message + "\\n_Ref: " + ref + "_";\n    setSentMessage(outbound);');

has('handleSend bails on a second click', 'function handleSend() {\n    if (sending) return;');
